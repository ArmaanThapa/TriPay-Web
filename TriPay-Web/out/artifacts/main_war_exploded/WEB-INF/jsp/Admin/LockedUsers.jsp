...
<sec:csrfMetaTags/>
<title>VPayQwik | Locked Users</title>
...
$.ajax({
type:"POST",
url:"${pageContext.request.contextPath}/Admin/LockedUsers",
data:{page:paging,size:'15'},
dataType:"json",
...
var trHTML='';
	...
	$(".testingg").empty();
	 ...
	 $(data.jsonArray).each(function(i,item){trHTML += '<tr class="testingg"><td>'+(i+1)+'</td><td>UserName:' + data.jsonArray[i].username + '<br>Mobileno.:'+ data.jsonArray[i].contactNo+'<br>Email id:'+ data.jsonArray[i].email+'</td>'+'<td>'+data.jsonArray[i].authority+'</td><td>Balance:'+data.jsonArray[i].balance+'<br>points:'+data.jsonArray[i].points+'</td></tr>'});
