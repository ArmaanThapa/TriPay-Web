...
<sec:csrfMetaTags />
<title>VPayQwik | Transactions Report</title>
...
function fetchMe(value){
	...
	$.ajax({
	type:"POST",
	url:"${pageContext.request.contextPath}/Admin/TransactionReportInJSON",
	...
	var trHTML='';
		...
		$(data.jsonArray).each(function(i,item){trHTML += '<tr class="testingg"><td>'+(i+1)+'</td><td>' + data.jsonArray[i].username + '<br><a href="${pageContext.request.contextPath}/Admin/User/'+data.jsonArray[i].contactNo+'">'+ data.jsonArray[i].contactNo+'</a><br>'+ data.jsonArray[i].currentBalance+'</td>'+'<td>'+ data.jsonArray[i].transactionRefNo+'</td><td>'+data.jsonArray[i].dateOfTransaction+'</td><td>'+data.jsonArray[i].description+'</td><td>'+data.jsonArray[i].amountReceivable+'</td><td>'+data.jsonArray[i].amountPayable+'</td><td>'+data.jsonArray[i].status+'</td></tr>';});
