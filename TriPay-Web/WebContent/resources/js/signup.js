...
//			$("#error_captcha").html("");
	...
	if(password.length == 0){
		...
		$("#error_password").html("password must be 6 to 12 characters long");
		valid = false;
	}
...
if(valid == true) {
	...
	$.ajax({
		...
		url : "/TriPay-web/Api/v1/User/Windows/en/WebRegistration/Process",
		dataType : 'json',
		data : JSON.stringify({
			"firstName" : "" + firstName+ "",
			"lastName" : " ",
...
//			$( ".captcha_link" ).trigger( "click" );
			...
			if (response.code.includes("S00")) {
				...
				//console.log((response.code.includes("F00")));
			     (response.code.includes("F00"))
			    
			     {
				if(response.details == null) {
						$("#error_captcha").html(response.message);
				}else{
					console.log("the rock star");
					var msg=[];
...
$("#register_resend_otp").click(function() {
	...
	$.ajax({
		...
		headers :headers,
		contentType : "application/json",
		url : "/TriPay-web/Api/v1/User/Windows/en/WebRegistration/ResendMobileOTP",
		dataType : 'json',
		data : JSON.stringify({
...
$("#register_verify_mobile").click(function() {
	...
	$.ajax({
		...
		headers : headers,
		contentType : "application/json",
		url : "/TriPay-web/Api/v1/User/Windows/en/Registration/MobileOTP",
		dataType : 'json',
		data : JSON.stringify({
			"mobileNumber" : "" + $('#reg_otp_username').val() + "",
...
$("#process_forgot_password_request").click(function() {
	...
	if(newPassword.length == 0){
		$("#error_fpPassword").html("enter Password");
		valid = false;
	}
	else if(newPassword.length <= 6){
		$("#error_fpPassword").html("password must be 6 to 12 characters long");
		valid = false;
	}
	...
	$.ajax({
		...
		headers : headers,
		contentType : "application/json",
		url : "/TriPay-web/ChangePasswordWithOTP",
		dataType : 'json',
		data : JSON.stringify({
			"username" : "" + $('#fpusername_forgot').val() + "",
